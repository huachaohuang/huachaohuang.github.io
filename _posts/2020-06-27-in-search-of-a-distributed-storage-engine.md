...
During compaction, the input files are merged, sorted, and compressed into new output files.

As for read, hot data are cached in memory to speed up performance.
A memtable is also a kind of cache, which holds the newly written data.
...
I also have to add more CPU and IO for other components so that I can run more instances of the storage engine.
...
We usually expect more from a distributed storage engine than a local one.
...
Moreover, a distributed storage engine should not limit the ability for domain-specific optimizations.
...
Now let's see how we can decompose the components of a local storage engine to a distributed storage engine.
...
This is why there are so many Redis API compatible storage systems, either open-source or homebrew.
...
With these considerations, the distributed cache system provides the flexibility for customization.
Each shard controls its API, cache, and memtable.
...
And you don't need to worry about cache consistency either, because all writes of a shard go to the same place in the cache system.
...
Since files in the distributed file system are static, they can be replicated by simply copying them to different places.
...
This adds some latency and potential bottleneck in the bandwidth.
But I don't very worry about it because the network is becoming more and more powerful, 100Gbps bandwidth is not uncommon.
Anyway, it is a good idea to add some kind of coprocessor mechanism to the distributed file system. Coprocessors let us push down the expressions to the data so that we don't need to transfer huge amounts of data through the network.

### Distributed Job System

Finally, we need a distributed job system to schedule different kinds of background jobs, like compaction.
